...
# CORRECTED VERSION (2026-08-28): the original class lacked the local term
# J r (E - V) in dV/dt (Eq. 25 of the paper); the connectome coupling cannot
# supply it because the structural matrix has a zero diagonal. The coupling
# variable is x, so the (R_minus/pi) factor on Coupling_Term must be kept.
class InfiniteHH(Model):
    ...
    def dfun(self, state_variables, coupling, local_coupling=0.0):
        ...
        # local recurrent term J r (E-V) of Eq. 25 added (missing in the original class)
        derivative[1,ind] = Vdot[ind] - R_minus*x[ind]**2 + eta + J*r[ind]*(E-V[ind]) + (R_minus/numpy.pi)*Coupling_Term[ind]*(E-V[ind])

        derivative[0,~ind] = Delta+2*R_plus*(V[~ind]-c_plus)*x[~ind]-J*r[~ind]*x[~ind] 
        derivative[1,~ind] = Vdot[~ind] - R_plus*x[~ind]**2 + eta + J*r[~ind]*(E-V[~ind]) + (R_minus/numpy.pi)*Coupling_Term[~ind]*(E-V[~ind])
